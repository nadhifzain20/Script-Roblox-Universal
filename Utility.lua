...
local UIBlur = Instance.new("BlurEffect", Lighting)
...
local function AddHoverEffect(btn)
    ...
    btn.MouseEnter:Connect(function()
        if btn.AutoButtonColor == false then
...
local function Notify(title, message, duration)
    ...
    for _, notif in ipairs(ActiveNotifications) do
        if notif and notif.Parent then 
            TweenService:Create(notif, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(1, -220, 1, notif.Position.Y.Offset - 65)}):Play() 
        end
    end
    local Notif = Instance.new("Frame", Gui)
    Notif.Size = UDim2.new(0, 200, 0, 60)
    Notif.Position = UDim2.new(1, 20, 1, -80)
    Notif.BackgroundColor3 = Theme.BackgroundTop
    ...
    TxtTitle.Text = title
    TxtTitle.TextColor3 = Theme.Text
    ...
    TxtMsg.Size = UDim2.new(1, -10, 0, 30)
    TxtMsg.Position = UDim2.new(0, 10, 0, 22)
    TxtMsg.Text = message
...
TooltipUI.BackgroundColor3 = Theme.BackgroundTop
TooltipUI.Size = UDim2.new(0, 200, 0, 30)
...
UIS.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement and TooltipUI.Visible then
        ...
        if now - lastTooltipUpdate > 0.05 then
            ...
            local targetX = input.Position.X + 15
            local targetY = input.Position.Y + 15
...
Open.Position = UDim2.new(0, 15, 0.5, -13)
Open.BackgroundTransparency = 1
Open.Text = "❤️"
Open.TextColor3 = Theme.Text
Open.TextSize = 20
Open.Selectable = false
CustomDrag(Open, Open)
...
StatsConnection = RunService.RenderStepped:Connect(function()
    ...
    if now - startTime >= 1 then 
        ...
        startTime = now 
        pcall(function() 
            lastPingValue = tonumber(StatsService.Network.ServerStatsItem["Data Ping"]:GetValueString():match("%d+")) or 0 
        end)
...
TabPanel.Size = UDim2.new(0, 100, 1, -36)
TabPanel.Position = UDim2.new(0, 0, 0, 36)
TabPanel.BackgroundColor3 = Theme.BackgroundTab
TabPanel.BorderSizePixel = 0
Corner(TabPanel, 12)

local TabPanelTopFixer = Instance.new("Frame", TabPanel)
TabPanelTopFixer.Size = UDim2.new(1, 0, 0, 12)
TabPanelTopFixer.BackgroundColor3 = Theme.BackgroundTab
TabPanelTopFixer.BorderSizePixel = 0
...
PageContainer.Size = UDim2.new(1, -100, 1, -36)
PageContainer.Position = UDim2.new(0, 100, 0, 36)
PageContainer.BackgroundTransparency = 1

local Tabs, CurrentTab = {}, nil

-- CreateTab with Search Bar Included
local function CreateTab(name, icon, layoutOrder)
    ...
    TabBtn.BackgroundColor3 = Theme.BackgroundTop
    ...
    Corner(TabBtn, 6)
    AddHoverEffect(TabBtn)
    
    local UIPad = Instance.new("UIPadding", TabBtn)
    ...
    SearchBox.BackgroundColor3 = Theme.BackgroundTop
    ...
    Page.ScrollBarImageColor3 = Theme.ButtonDefault
    ...
    PageSpacer.Size = UDim2.new(1, 0, 0, 2)
    PageSpacer.BackgroundTransparency = 1
    PageSpacer.LayoutOrder = 0
    
    UIList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() 
        Page.CanvasSize = UDim2.new(0, 0, 0, UIList.AbsoluteContentSize.Y + 15) 
    end)
        ...
        for _, child in pairs(Page:GetChildren()) do
            if child:IsA("GuiObject") and child ~= PageSpacer then
                if child:IsA("TextButton") then
                    child.Visible = (txt == "" or string.find(string.lower(child.Text), txt) ~= nil)
                elseif child:IsA("Frame") then
                    local btn = child:FindFirstChildOfClass("TextButton")
                    if btn then
                        child.Visible = (txt == "" or string.find(string.lower(btn.Text), txt) ~= nil)
                    end
                end
            end
        end
    ...
    TabBtn.Activated:Connect(function()
        ...
        for _, v in pairs(Tabs) do 
            ...
            v.Btn.BackgroundColor3 = Theme.BackgroundTop
            v.Btn.TextColor3 = Color3.new(1,1,1)
            TweenService:Create(v.Ind, TweenInfo.new(0.3), {Size = UDim2.new(0, 3, 0, 0)}):Play() 
        end
        PageWrapper.Visible = true
        TabBtn.BackgroundColor3 = Theme.ButtonDefault
...
-- Improved Toggle UI (Bounce Animation)
local UI_Open = false
local function ToggleUI()
    ...
    if UI_Open then 
        Frame.Visible = true 
        Frame.Size = UDim2.new(0, 360, 0, 0)
        TweenService:Create(Frame, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 360, 0, 260)}):Play()
        TweenService:Create(UIBlur, TweenInfo.new(0.4), {Size = 20}):Play()
    else
        local tw = TweenService:Create(Frame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Size = UDim2.new(0, 360, 0, 0)})
        tw:Play()
        TweenService:Create(UIBlur, TweenInfo.new(0.3), {Size = 0}):Play()
        tw.Completed:Connect(function()
            if not UI_Open then Frame.Visible = false end
        end)
    end
end
...
Corner(Close, 6)
Close.Activated:Connect(ToggleUI)
Open.Activated:Connect(ToggleUI)

local ThemeToggle = Instance.new("TextButton", Top)
ThemeToggle.Size = UDim2.new(0, 26, 0, 26)
ThemeToggle.Position = UDim2.new(1, -66, 0.5, -13)
ThemeToggle.Text = "☀️"
...
ThemeToggle.Activated:Connect(function()
    ...
    ThemeToggle.Text = IsDarkMode and "☀️" or "🌙"
    local from = IsDarkMode and Themes.Light or Themes.Dark
    local to = IsDarkMode and Themes.Dark or Themes.Light
    Theme = to
    local colorMap = {
        [from.Background:ToHex()] = to.Background, 
        [from.BackgroundTop:ToHex()] = to.BackgroundTop, 
        [from.BackgroundTab:ToHex()] = to.BackgroundTab, 
        [from.ButtonDefault:ToHex()] = to.ButtonDefault, 
        [from.ButtonOn:ToHex()] = to.ButtonOn, 
        [from.ButtonOff:ToHex()] = to.ButtonOff, 
        [from.Text:ToHex()] = to.Text
    }
    for _, obj in pairs(Gui:GetDescendants()) do
        pcall(function()
            if obj:IsA("GuiObject") and colorMap[obj.BackgroundColor3:ToHex()] and obj ~= ThemeToggle and obj ~= LangToggle then 
                TweenService:Create(obj, TweenInfo.new(0.3), {BackgroundColor3 = colorMap[obj.BackgroundColor3:ToHex()]}):Play() 
            end
    ...
    for _, v in pairs(Tabs) do
        if CurrentTab == v.Name then 
            TweenService:Create(v.Btn, TweenInfo.new(0.3), {BackgroundColor3 = Theme.ButtonDefault, TextColor3 = Theme.Text}):Play() 
        else 
            TweenService:Create(v.Btn, TweenInfo.new(0.3), {BackgroundColor3 = Theme.BackgroundTop, TextColor3 = Color3.new(1,1,1)}):Play() 
        end
...
LangToggle.Size = UDim2.new(0, 26, 0, 26)
LangToggle.Position = UDim2.new(1, -98, 0.5, -13)
LangToggle.Text = "友"
LangToggle.TextSize = 16
LangToggle.Font = Enum.Font.GothamBold
LangToggle.AutoButtonColor = false
LangToggle.BackgroundTransparency = 1
...
LangToggle.Activated:Connect(function()
    ...
    for _, data in ipairs(DynamicLabels) do 
        if data.Obj then data.Obj.Text = CurrentLanguage == "ID" and data.ID or data.EN end 
    end
end)
...
-- UI COMPONENTS
------------------------------------------------
local function GetInfoString(info)
    return (type(info) == "table") and (info[CurrentLanguage] or info.ID) or info
end

local function CreateButton(parent, text, color, layoutOrder, infoText)
    local MainBtn = Instance.new("TextButton", parent)
    MainBtn.Size = UDim2.new(0, 220, 0, 28)
    MainBtn.BackgroundColor3 = color
    MainBtn.Text = text
    MainBtn.TextColor3 = Theme.Text
    MainBtn.TextSize = 11
    MainBtn.Font = Enum.Font.GothamBold
    MainBtn.AutoButtonColor = false
    MainBtn.LayoutOrder = layoutOrder
    Corner(MainBtn, 8)
    ...
    if infoText then
        ...
        InfoBtn.Position = UDim2.new(1, -30, 0, 0)
        ...
        InfoBtn.MouseEnter:Connect(function()
            ...
            TooltipText.Text = GetInfoString(infoText)
            TooltipText.Size = UDim2.new(1, -10, 0, 1000)
        ...
        InfoBtn.Activated:Connect(function() 
            TooltipUI.Visible = false
            Notify("INFO", GetInfoString(infoText), 6) 
        end)
    end
    return MainBtn
end
...
local function CreateSlider(parent, name, min, max, default, color, layoutOrder, callback)
    ...
    Holder.Size = UDim2.new(0, 220, 0, 42)
    ...
    VBar.Size = UDim2.new(1, 0, 0, 4)
    VBar.Position = UDim2.new(0, 0, 0.5, -2)
    VBar.BackgroundColor3 = Color3.fromHex("#6237A0")
    Corner(VBar, 999)
    
    local Fill = Instance.new("Frame", VBar)
    ...
    UIS.InputChanged:Connect(function(input) 
        if ActiveSlider == Handle and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then 
            Update(input.Position.X) 
        end 
    end)
    UIS.InputEnded:Connect(function(input) 
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then 
            if ActiveSlider == Handle then 
                ActiveSlider = nil
    ...
    Bar.InputEnded:Connect(function(input) 
        if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and startX then 
            if math.abs(input.Position.X - startX) < 5 and ActiveSlider == nil then Update(input.Position.X) end
            startX = nil 
        end 
    end)
...
local function CreateAccordion(parent, title, color, layoutOrder, infoText, maxHeight)
    ...
    local Holder = Instance.new("Frame", parent)
    Holder.Size = UDim2.new(0, 220, 0, 28)
    Holder.BackgroundTransparency = 1
    ...
    Scroll.ScrollBarImageColor3 = Theme.ButtonDefault
    ...
    UIS.InputEnded:Connect(function(input) 
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then 
            if ActiveScroll == Scroll then 
                ActiveScroll = nil
                if parent:IsA("ScrollingFrame") then parent.ScrollingEnabled = true end 
            end 
        end 
    end)

    local function UpdateSize() 
        TweenService:Create(Holder, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 220, 0, State.IsOpen and (32 + math.clamp(ListUI.AbsoluteContentSize.Y + State.ExtraHeight, 0, maxHeight)) or 28)}):Play() 
    end
    ToggleBtn.Activated:Connect(function() 
        State.IsOpen = not State.IsOpen
        ToggleBtn.Text = State.IsOpen and (title .. " ▲") or (title .. " ▼")
        UpdateSize() 
    end)
...
BtnProfile.BackgroundColor3 = Theme.ButtonDefault
BtnProfile.TextColor3 = Theme.Text
IndProf.Size = UDim2.new(0, 3, 0.6, 0)
WrapProf.Visible = true
CurrentTab = "Profile"
...
local function ApplyProperties(char)
    ...
    if hum then
        ...
        -- Death Tracker for Teleport Back
        hum.Died:Connect(function()
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if hrp then
                LastPositionCFrame = hrp.CFrame
                Notify("DEATH", "Posisi terakhir disimpan. Tekan 'Teleport Back' untuk kembali.", 5)
            end
        end)
    end
end
...
ProfileListUI.SortOrder = Enum.SortOrder.LayoutOrder
ProfileListUI.Padding = UDim.new(0, 10)
ProfileListUI.HorizontalAlignment = Enum.HorizontalAlignment.Center
ProfileListUI:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() 
    TabProfile.CanvasSize = UDim2.new(0, 0, 0, ProfileListUI.AbsoluteContentSize.Y + 20) 
end)
...
local function CreateProfileAccordion(title, layoutOrder, maxHeight)
    ...
    ListUI:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() 
        Scroll.CanvasSize = UDim2.new(0, 0, 0, ListUI.AbsoluteContentSize.Y)
        if State.IsOpen then UpdateSize() end 
    end)
    return Scroll
end
...
AvatarFrame.BackgroundColor3 = Theme.BackgroundTop
AvatarFrame.LayoutOrder = 1
Corner(AvatarFrame, 32)
...
coroutine.wrap(function() 
    local s, img = pcall(function() return Players:GetUserThumbnailAsync(LP.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420) end)
    if s then AvatarImg.Image = img end 
end)()
...
local ValPremium = CreateRow(SecAcc, "Premium")
ValPremium.Text = LP.MembershipType == Enum.MembershipType.Premium and "Yes" or "No"
ValPremium.TextColor3 = LP.MembershipType == Enum.MembershipType.Premium and Color3.fromHex("#F1C40F") or Color3.new(1, 1, 1)
...
coroutine.wrap(function() 
    local api = GetAPI("https://users.roproxy.com/v1/users/" .. LP.UserId)
    ValBio.Text = api and (api.description ~= "" and api.description or "No Bio") or "API Blocked" 
end)()
coroutine.wrap(function() 
    local api = GetAPI("https://friends.roproxy.com/v1/users/" .. LP.UserId .. "/friends/count")
    ValFriends.Text = api and tostring(api.count or 0) or "Error" 
end)()
coroutine.wrap(function() 
    local s, online = pcall(function() return LP:GetFriendsOnline(200) end)
    ValActiveFriends.Text = s and tostring(#online) or "Error" 
end)()
...
coroutine.wrap(function() 
    ...
    ValGame.Text = s and info.Name or "Unknown Game" 
end)()
...
local ValStatus = CreateRow(SecExtra, "Status")
ValStatus.Text = "Connected"
ValStatus.TextColor3 = Color3.fromHex("#2ECC71")

-- // 2. MOVEMENT TAB
local SetSpeedVisual = CreateSlider(TabMovement, "Walk Speed", 16, 120, 16, Theme.Text, 1, function(v) 
    WalkSpeed = v
    local h = Humanoid()
    if h then h.WalkSpeed = v end 
end)
local SetJumpVisual = CreateSlider(TabMovement, "Jump Power", 50, 250, 50, Theme.Text, 2, function(v) 
    JumpPower = v
    local h = Humanoid()
...
local function SetInfJump(state) 
    ...
    InfBtn.Text = state and "Inf Jump : ON" or "Inf Jump : OFF"
    InfBtn.BackgroundColor3 = state and Theme.ButtonOn or Theme.ButtonOff 
    AddHoverEffect(InfBtn)
end
...
Connections.Jump = UIS.JumpRequest:Connect(function() 
    if ToggleStates.InfJump and Humanoid() then Humanoid():ChangeState(Enum.HumanoidStateType.Jumping) end 
end)
...
local function SetNoclip(state) 
    ...
    NoclipBtn.BackgroundColor3 = state and Theme.ButtonOn or Theme.ButtonOff 
    AddHoverEffect(NoclipBtn)
    if state then
        if not Connections.Noclip then
            Connections.Noclip = RunService.Stepped:Connect(function()
                if LP.Character then
                    if LP.Character ~= Connections.LastCharNoclip then 
                        Connections.LastCharNoclip = LP.Character
                        Connections.CharPartsCache = {}
                        for _, v in ipairs(LP.Character:GetDescendants()) do 
                            if v:IsA("BasePart") then table.insert(Connections.CharPartsCache, v) end 
                        end 
                    end
                    for i = 1, #Connections.CharPartsCache do 
                        local part = Connections.CharPartsCache[i]
                        if part and part.CanCollide then part.CanCollide = false end 
                    end
                end
            end)
        end
        ...
        if Connections.Noclip then 
            ...
            Connections.Noclip, Connections.LastCharNoclip, Connections.CharPartsCache = nil, nil, nil 
        end 
    end
end
...
local function SetFly(state)
    ToggleStates.Fly = state
    local h, hum = HRP(), Humanoid()
    if not h or not hum then ToggleStates.Fly = false return end
    
    if state then
        PrevNoclipState = ToggleStates.Noclip
        if not ToggleStates.Noclip then SetNoclip(true) end -- Auto Noclip On
        hum.PlatformStand = true
        FlyBody = Instance.new("BodyVelocity", h)
        FlyBody.MaxForce = Vector3.new(9e9, 9e9, 9e9)
        FlyBody.Velocity = Vector3.zero
        FlyGyro = Instance.new("BodyGyro", h)
        FlyGyro.MaxTorque = Vector3.new(9e9, 9e9, 9e9)
        FlyGyro.P = 9e4
        FlyGyro.CFrame = h.CFrame
        ...
        if not PrevNoclipState and ToggleStates.Noclip then SetNoclip(false) end -- Revert Auto Noclip
    end
end
local FlyBtn = CreateButton(TabMovement, "Fly : OFF", Theme.ButtonOff, 5, {ID="Terbang bebas di udara.", EN="Fly freely."})
FlyBtn.Activated:Connect(function() 
    SetFly(not ToggleStates.Fly)
    FlyBtn.Text = ToggleStates.Fly and "Fly : ON" or "Fly : OFF"
    FlyBtn.BackgroundColor3 = ToggleStates.Fly and Theme.ButtonOn or Theme.ButtonOff
    AddHoverEffect(FlyBtn)
end)
...
EmoteBtn.Activated:Connect(function() 
    Notify("SYSTEM", CurrentLanguage == "ID" and "Memuat script..." or "Loading script...", 3)
...
-- // 3. VISUAL TAB
local SetFOVVisual = CreateSlider(TabVisual, "Field Of View", 70, 120, 70, Theme.Text, 1, function(v) 
    CurrentFOV = v
...
local function SetMaxZoom(state) 
    ToggleStates.MaxZoom = state
    MaxZoomBtn.Text = state and "Max Zoom : ON" or "Max Zoom : OFF"
    MaxZoomBtn.BackgroundColor3 = state and Theme.ButtonOn or Theme.ButtonOff
    AddHoverEffect(MaxZoomBtn)
    LP.CameraMaxZoomDistance = state and 100000 or 400 
end
...
local function SetPotatoMode(state)
    ...
    if state then
        ...
        FPSBtn.BackgroundColor3 = Theme.ButtonOn
        AddHoverEffect(FPSBtn)
        Notify("SYSTEM", "Optimizing game...", 3)
        ...
        local function ApplyPotato(v)
            ...
            if v:IsA("BasePart") and not v:IsA("MeshPart") then 
                if not OriginalGraphics[v].Material then OriginalGraphics[v].Material = v.Material; OriginalGraphics[v].Reflectance = v.Reflectance end
                v.Material = Enum.Material.SmoothPlastic
                v.Reflectance = 0
            elseif v:IsA("Decal") or v:IsA("Texture") then 
                if not OriginalGraphics[v].Transparency then OriginalGraphics[v].Transparency = v.Transparency end
                v.Transparency = 1
            elseif v:IsA("ParticleEmitter") or v:IsA("Trail") then 
                if not OriginalGraphics[v].Lifetime then OriginalGraphics[v].Lifetime = v.Lifetime end
                v.Lifetime = NumberRange.new(0) 
            end
        end
        coroutine.wrap(function() 
            local des = workspace:GetDescendants()
            for i = 1, #des do ApplyPotato(des[i]); if i % 500 == 0 then RunService.Heartbeat:Wait() end end 
        end)()
        ...
        FPSBtn.BackgroundColor3 = Theme.ButtonOff
        AddHoverEffect(FPSBtn)
        Notify("SYSTEM", "Restoring graphics...", 3)
        ...
        for v, data in pairs(OriginalGraphics) do 
            if v and v.Parent then 
                if data.Material then v.Material = data.Material; v.Reflectance = data.Reflectance end
                if data.Transparency then v.Transparency = data.Transparency end
                if data.Lifetime then v.Lifetime = data.Lifetime end 
            end 
        end
    end
end
        ...
        ESPConnection = RunService.RenderStepped:Connect(function()
            for player, espData in pairs(ESPInstances) do
                if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
                    ...
                    espData.Billboard.Parent = player.Character.HumanoidRootPart
                    local myHRP = HRP()
                    if myHRP then
                        ...
                        espData.Text.RichText = true
                    end
                else
                    espData.Highlight.Parent = nil
                    espData.Billboard.Parent = nil
                end
            end
        end)
    else
...
local function SetInstantPrompt(state) 
    ToggleStates.InstantPrompt = state
    InstantBtn.Text = state and "Instant Prompt : ON" or "Instant Prompt : OFF"
    InstantBtn.BackgroundColor3 = state and Theme.ButtonOn or Theme.ButtonOff
    AddHoverEffect(InstantBtn)
end
InstantBtn.Activated:Connect(function() SetInstantPrompt(not ToggleStates.InstantPrompt) end)
Connections.Prompt = ProximityPromptService.PromptButtonHoldBegan:Connect(function(p) 
    if ToggleStates.InstantPrompt then 
        p.HoldDuration = 0
...
local function SetAntiAFK(state) 
    ToggleStates.AntiAFK = state
    AntiAFKBtn.Text = state and "Anti AFK : ON" or "Anti AFK : OFF"
    AntiAFKBtn.BackgroundColor3 = state and Theme.ButtonOn or Theme.ButtonOff
    AddHoverEffect(AntiAFKBtn)
end
AntiAFKBtn.Activated:Connect(function() SetAntiAFK(not ToggleStates.AntiAFK) end)
Connections.Idled = LP.Idled:Connect(function()
    if ToggleStates.AntiAFK then
        local cam = workspace.CurrentCamera
        VirtualUser:CaptureController()
        VirtualUser:ClickCenter(Vector2.new(0, 0), cam.CFrame)
    end
end)
...
KeybindBtn.Activated:Connect(function() 
    ...
    KeybindBtn.BackgroundColor3 = Color3.fromHex("#E74C3C")
    AddHoverEffect(KeybindBtn)
end)
Connections.Input = UIS.InputBegan:Connect(function(input, gpe)
    if Binding and input.UserInputType == Enum.UserInputType.Keyboard then 
        ...
        KeybindBtn.BackgroundColor3 = Theme.ButtonDefault
        AddHoverEffect(KeybindBtn)
        Notify("SYSTEM", "Keybind changed", 3)
    elseif not gpe and input.KeyCode == CurrentKeybind then ToggleUI() end
end)

-- SMART PART INSPECTOR
...
local function SetSmartInspector(state)
    ...
    AddHoverEffect(SmartInspectorBtn)
...
local function RefreshPlayers()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP then
            ...
            btn.BackgroundColor3 = Theme.BackgroundTop
            ...
            btn.Activated:Connect(function() 
                local my, tH = HRP(), p.Character and p.Character:FindFirstChild("HumanoidRootPart")
                if my and tH then my.CFrame = tH.CFrame; Notify("TELEPORT", "Teleported to " .. p.DisplayName, 3) end 
            end)
        end
    end
...
WPScroll.Size = UDim2.new(1, 0, 1, -32)
WPScroll.Position = UDim2.new(0, 0, 0, 32)
WPState.ExtraHeight = 32

local AddWPBtn = CreateButton(WPMenu, "+ Add Current Pos", Theme.ButtonOn, 1)
AddWPBtn.Size = UDim2.new(1, 0, 0, 28)
AddWPBtn.Position = UDim2.new(0, 0, 0, 0)
AddWPBtn.TextColor3 = Color3.new(1,1,1)
AddHoverEffect(AddWPBtn)

local WPContainer = Instance.new("Frame", Gui)
WPContainer.Size = UDim2.new(0, 60, 0, 200)
WPContainer.Position = UDim2.new(0, 10, 0, 10)
...
local function RefreshFloatingWPs()
    ...
    for id, cf in pairs(Waypoints) do
        ...
        btn.Position = UDim2.new(0, 0, 0, (idx-1)*35)
        btn.Text = "WP "..id
        ...
        Corner(btn, 8)
        AddHoverEffect(btn)
        btn.Activated:Connect(function() local h = HRP(); if h then h.CFrame = cf end end)
        CustomDrag(btn, WPContainer)
        idx = idx + 1
    end
end
...
local function CreateDynamicWP(id, cfToSave)
    ...
    local Item = Instance.new("Frame", WPScroll)
    Item.Size = UDim2.new(1, 0, 0, 30)
    Item.BackgroundColor3 = Theme.BackgroundTop
    Corner(Item, 8)
    ...
    TpBtn.Text = "Go"
    TpBtn.BackgroundColor3 = Theme.ButtonDefault
    TpBtn.TextColor3 = Theme.Text
    TpBtn.Font = Enum.Font.GothamBold
    TpBtn.AutoButtonColor = false
    Corner(TpBtn, 6)
    AddHoverEffect(TpBtn)
    
    local EditBtn = Instance.new("TextButton", Item)
    EditBtn.Size = UDim2.new(0, 35, 0, 22)
    EditBtn.Position = UDim2.new(1, -70, 0.5, -11)
    EditBtn.Text = "Set"
    EditBtn.BackgroundColor3 = Color3.fromHex("#F39C12")
    ...
    DelBtn.Position = UDim2.new(1, -30, 0.5, -11)
    DelBtn.Text = "X"
        ...
        if s and res and res.data then
            for _, srv in pairs(res.data) do
                if srv.playing and srv.playing < srv.maxPlayers and srv.id ~= game.JobId then
                    Notify("SERVER", CurrentLanguage == "ID" and "OTW pindah..." or "Teleporting...", 3)
...
local HopBtnContainer = Instance.new("Frame", HopScroll)
HopBtnContainer.Size = UDim2.new(1, -6, 0, 28)
HopBtnContainer.BackgroundTransparency = 1
HopBtnContainer.LayoutOrder = 1

local HopEmptyBtn = CreateButton(HopBtnContainer, "Hop (Sepi)", Color3.fromHex("#1D4ED8"), 1)
HopEmptyBtn.Size = UDim2.new(0.5, -2, 1, 0)
HopEmptyBtn.TextColor3 = Color3.new(1, 1, 1)
HopEmptyBtn.Activated:Connect(function() PerformServerHop(false) end)
RegisterDynamicLang(HopEmptyBtn, "Hop (Sepi)", "Hop (Empty)")

local HopCrowdedBtn = CreateButton(HopBtnContainer, "Hop (Rame)", Color3.fromHex("#BE185D"), 2)
HopCrowdedBtn.Size = UDim2.new(0.5, -2, 1, 0)
HopCrowdedBtn.AnchorPoint = Vector2.new(1, 0)
HopCrowdedBtn.Position = UDim2.new(1, 0, 0, 0)
HopCrowdedBtn.TextColor3 = Color3.new(1, 1, 1)
HopCrowdedBtn.Activated:Connect(function() PerformServerHop(true) end)
RegisterDynamicLang(HopCrowdedBtn, "Hop (Rame)", "Hop (Crowded)")
HopScroll.CanvasSize = UDim2.new(0, 0, 0, 30)

local RejoinBtn = CreateButton(TabTeleport, "Rejoin", Color3.fromHex("#E74C3C"), 4, {ID="Masuk ulang server sama.", EN="Rejoin same server."})
...
-- Copy Job ID Button
local CopyIdBtn = CreateButton(TabTeleport, "Copy Job ID", Color3.fromHex("#2563EB"), 5, {ID="Salin ID Server.", EN="Copy Server ID."})
...
-- Teleport Back (Death Position)
local TpBackBtn = CreateButton(TabTeleport, "Teleport Back (Death)", Color3.fromHex("#9333EA"), 6, {ID="Kembali ke posisi sebelum mati.", EN="Go back to position before death."})
TpBackBtn.Activated:Connect(function()
    local h = HRP()
    if h and LastPositionCFrame then
        h.CFrame = LastPositionCFrame
        Notify("TELEPORT", "Berhasil kembali ke posisi terakhir!", 3)
    else
        Notify("ERROR", "Belum ada catatan kematian.", 3)
    end
end)

-- // 6. CONFIG TAB
local ConfigInput = Instance.new("TextBox", TabConfig)
ConfigInput.Size = UDim2.new(0, 220, 0, 28)
ConfigInput.BackgroundColor3 = Theme.BackgroundTop
ConfigInput.TextColor3 = Theme.Text
ConfigInput.Font = Enum.Font.GothamBold
ConfigInput.TextSize = 11
ConfigInput.PlaceholderText = "(nama config)"
ConfigInput.Text = ""
ConfigInput.LayoutOrder = 1
Corner(ConfigInput, 8)

local SaveBtn = CreateButton(TabConfig, "Save Config", Color3.fromHex("#0F766E"), 2, {ID="Simpan setingan.", EN="Save settings."})
...
local function GetConfigPath(name)
    return ConfigFolder .. "/" .. string.gsub(name=="" and "ConfigUtama" or name, "[^%w_]", "") .. ".json"
end
...
local function LoadConfigData(path)
    if readfile and isfile and isfile(path) then
        ...
        if s and res then
            WalkSpeed = res.WalkSpeed or 16
            JumpPower = res.JumpPower or 50
            CurrentFOV = res.FOV or 70
            FlySpeed = res.FlySpeed or 50
            CurrentKeybind = Enum.KeyCode[res.Keybind or "RightControl"]
            KeybindBtn.Text = "Keybind : " .. CurrentKeybind.Name
            SetSpeedVisual(WalkSpeed)
            SetJumpVisual(JumpPower)
            SetFOVVisual(CurrentFOV)
            SetFlySpeedVisual(FlySpeed)
            if res.InfJump ~= ToggleStates.InfJump then SetInfJump(res.InfJump) end
            if res.Noclip ~= ToggleStates.Noclip then SetNoclip(res.Noclip) end
            if res.InstantPrompt ~= ToggleStates.InstantPrompt then SetInstantPrompt(res.InstantPrompt) end
            if res.MaxZoom ~= ToggleStates.MaxZoom then SetMaxZoom(res.MaxZoom) end
            if res.AntiAFK ~= ToggleStates.AntiAFK then SetAntiAFK(res.AntiAFK) end
            if res.PotatoMode ~= ToggleStates.PotatoMode then SetPotatoMode(res.PotatoMode) end
...
SaveBtn.Activated:Connect(function()
    if writefile then
        local data = { 
            WalkSpeed = WalkSpeed, JumpPower = JumpPower, FOV = CurrentFOV, FlySpeed = FlySpeed, 
            Keybind = CurrentKeybind.Name, InfJump = ToggleStates.InfJump, Noclip = ToggleStates.Noclip, 
            InstantPrompt = ToggleStates.InstantPrompt, MaxZoom = ToggleStates.MaxZoom, AntiAFK = ToggleStates.AntiAFK, 
            PotatoMode = ToggleStates.PotatoMode, ESP = ToggleStates.ESP, Fullbright = ToggleStates.Fullbright, 
            Nofog = ToggleStates.Nofog, Waypoints = {} 
        }
...
local UnloadBtn = CreateButton(TabConfig, "Unload Script", Color3.fromHex("#C0392B"), 4, {ID="Hapus script dari layar.", EN="Remove script."})
...
coroutine.wrap(function()
    ...
    sound.Volume = 0.5
    sound:Play()
    Notify("SYSTEM", "Mamet Utility Pro V7.17.5 Loaded", 5)
